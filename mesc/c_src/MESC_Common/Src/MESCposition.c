...
void RunPosControl(MESC_motor_typedef* _motor) {
    ...
    //	if(_motor->pos.set_position<200000){_motor->pos.set_position =
    //_motor->pos.set_position+1500;}
    ...
    // Clamp the integral
    if (_motor->pos.int_error > _motor->input_vars.max_request_Idq.q) {
        _motor->pos.int_error = _motor->input_vars.max_request_Idq.q;
    }
    if (_motor->pos.int_error < _motor->input_vars.min_request_Idq.q) {
        _motor->pos.int_error = _motor->input_vars.min_request_Idq.q;
    }
    ...
    if (fabsf(_motor->pos.error) > _motor->pos.deadzone) {
        _motor->FOC.Idq_prereq.q =
            _motor->pos.p_error + _motor->pos.int_error + _motor->pos.d_error;
    } else {
        _motor->FOC.Idq_prereq.q = 0.0f;
    }
    // Clamp the output
    if (_motor->FOC.Idq_prereq.q > _motor->input_vars.max_request_Idq.q) {
        _motor->FOC.Idq_prereq.q = _motor->input_vars.max_request_Idq.q;
    }
    if (_motor->FOC.Idq_prereq.q < _motor->input_vars.min_request_Idq.q) {
